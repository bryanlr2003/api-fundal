...
function pick(colset, candidates) {
...
async function discoverShape() {
    ...
    if (!table)
        throw new Error('No se encontró la tabla de pacientes');
...
function normalizeRole(raw) {
    ...
    if (['ADMIN', 'SUPERADMIN', 'ADMINISTRADOR', 'SUPER ADMIN', 'SUPER_ADMIN', 'ROOT'].includes(r))
        return 'ADMINISTRADOR';
    if (['TERAPEUTA', 'THERAPIST'].includes(r))
        return 'TERAPEUTA';
    return '';
}
// === GET /pacientes ===
...
router.post('/', requireAuth, async (req, res) => {
    try {
        ...
        const ph = [];
        if (map.nombres) {
            cols.push(map.nombres);
            vals.push(String(body.nombres).trim());
            ph.push(`$${vals.length}`);
        }
        if (map.apellidos) {
            cols.push(map.apellidos);
            vals.push(String(body.apellidos).trim());
            ph.push(`$${vals.length}`);
        }
        if (map.sexo) {
            cols.push(map.sexo);
        ...
        if (map.fecha_nacimiento) {
            cols.push(map.fecha_nacimiento);
            vals.push(body.fecha_nacimiento ?? null);
            ph.push(`$${vals.length}`);
        }
        if (map.terapeuta_id) {
            cols.push(map.terapeuta_id);
            vals.push(terId);
            ph.push(`$${vals.length}`);
        }
        if (map.activo) {
            cols.push(map.activo);
            vals.push(true);
            ph.push(`$${vals.length}`);
        }
        if (!cols.length)
            return res.status(400).json({ error: 'No hay columnas coincidentes para insertar' });
        const selCreado = map.creado ? map.creado : 'NULL';
        const selActual = map.actualizado ? map.actualizado : selCreado;
        const returning = [
            map.id ? `${map.id} AS id` : 'NULL AS id',
...
router.put('/:id', requireAuth, async (req, res) => {
    try {
        ...
        if (map.nombres && typeof body.nombres === 'string') {
            sets.push(`${map.nombres} = $${idx++}`);
            vals.push(body.nombres.trim());
        }
        if (map.apellidos && typeof body.apellidos === 'string') {
            sets.push(`${map.apellidos} = $${idx++}`);
            vals.push(body.apellidos.trim());
        }
        if (map.edad && (body.edad === null || Number.isFinite(Number(body.edad)))) {
            sets.push(`${map.edad} = $${idx++}`);
            vals.push(body.edad == null ? null : Number(body.edad));
        }
        if (map.fecha_nacimiento && (body.fecha_nacimiento === null || typeof body.fecha_nacimiento === 'string')) {
            sets.push(`${map.fecha_nacimiento} = $${idx++}`);
            vals.push(body.fecha_nacimiento ?? null);
        }
        ...
        if (isAdmin && map.terapeuta_id && (body.terapeuta_id === null || Number.isFinite(Number(body.terapeuta_id)))) {
            sets.push(`${map.terapeuta_id} = $${idx++}`);
            vals.push(body.terapeuta_id == null ? null : Number(body.terapeuta_id));
        }
        if (!sets.length)
