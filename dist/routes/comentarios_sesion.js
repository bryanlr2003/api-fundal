...
router.post('/:id/comentarios', requireAuth, async (req, res) => {
    ...
    `;
        ...
        try {
            await pool.query(`INSERT INTO auditoria (usuario_id, accion, entidad, entidad_id, detalle)
         VALUES ($1, $2, $3, $4, $5::jsonb)`, [autorId, 'CREAR_COMENTARIO_SESION', 'comentarios_sesion', r.rows[0].id,
 ...
 */
router.post('/comentarios', requireAuth, async (req, res) => {
    const pool = req.app.get('db');
    const autorId = Number(req.user?.id);
    const { pacienteId, comentario } = (req.body ?? {});
    ...
    `;
        ...
        try {
            await client.query(`INSERT INTO auditoria (usuario_id, accion, entidad, entidad_id, detalle)
         VALUES ($1, $2, $3, $4, $5::jsonb)`, [autorId, 'CREAR_BITACORA_COMENTARIO', 'comentarios_sesion', cmtR.rows[0].id,
...
router.get('/:id/comentarios', requireAuth, async (req, res) => {
    ...
    if (!Number.isFinite(sesionId))
        return res.status(400).json({ error: 'ID de sesión inválido' });
    try {
        const r = await pool.query(`SELECT id, sesion_id, autor_id, texto, timestamp, fecha_crea, fecha_modifica
         FROM comentarios_sesion
        WHERE sesion_id = $1
        ORDER BY fecha_crea DESC`, [sesionId]);
