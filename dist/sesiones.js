...
async function discoverShape() {
    ...
    if (!table)
        throw new Error('No se encontró la tabla de notas/bitácora (ajusta TABLE_CANDIDATES).');
...
router.post('/', requireAuth, async (req, res) => {
    try {
        ...
        const vals = [];
        if (map.paciente_id) {
            cols.push(map.paciente_id);
            vals.push(pacienteId);
            ph.push(`$${vals.length}`);
        }
        if (map.terapeuta_id && terapeutaId != null) {
            cols.push(map.terapeuta_id);
            vals.push(terapeutaId);
            ph.push(`$${vals.length}`);
        }
        if (map.titulo && titulo != null) {
            cols.push(map.titulo);
            vals.push(String(titulo).trim());
            ph.push(`$${vals.length}`);
        }
        if (map.nota) {
            cols.push(map.nota);
            vals.push(String(nota).trim());
            ph.push(`$${vals.length}`);
        }
        if (map.fecha) {
            cols.push(map.fecha);
        ...
        if (!cols.length)
            return res.status(500).json({ error: 'No hay columnas coincidentes para insertar la nota' });
        const returning = [
            map.id ? `${map.id} as id` : 'NULL as id',
            map.paciente_id ? `${map.paciente_id} as paciente_id` : 'NULL as paciente_id',
            map.terapeuta_id ? `${map.terapeuta_id} as terapeuta_id` : 'NULL as terapeuta_id',
