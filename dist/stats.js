...
function pick(set, arr) { for (const c of arr)
    if (set.has(c))
        return c; return null; }
function isDateLike(ci) { if (!ci)
    return false; const t = ci.data_type?.toLowerCase?.() ?? ''; return t.includes('timestamp') || t === 'date'; }
async function discover(tableCandidates, colsMap) {
    let table = null;
    let cols = [];
    ...
    for (const key of Object.keys(colsMap))
        map[key] = pick(set, colsMap[key] ?? []);
    const byName = Object.fromEntries(cols.map(c => [c.column_name, c]));
    return { table, map, set, byName };
}
function mapTipoModulo(raw) {
    const t = (raw || '').toUpperCase();
    if (t === 'A' || t.startsWith('ULTRA'))
        return 'ULTRASONICOS';
    if (t === 'B' || t.startsWith('PULS'))
        return 'PULSADORES';
    return t;
}
// ========== OVERVIEW ==========
    ...
    try {
        ...
        if (pac.table && pac.map.id) {
            if (pac.map.owner_terapeuta_id) {
                const { rows } = await pool.query(`SELECT COUNT(*)::int AS c FROM ${pac.table} WHERE ${pac.map.owner_terapeuta_id}=$1`, [terapeutaId]);
                pacientesTotal = rows[0]?.c ?? 0;
            }
            else if (ses.table && ses.map.paciente_id && ses.map.terapeuta_id) {
                const { rows } = await pool.query(`SELECT COUNT(DISTINCT p.${pac.map.id})::int AS c
             FROM ${pac.table} p
            WHERE EXISTS (
        ...
        if (run.table && run.map.tipo && run.map.terapeuta_id && run.map.inicio && isDateLike(run.byName[run.map.inicio])) {
            const { rows } = await pool.query(`SELECT ${run.map.tipo} as tipo, COUNT(*)::int as total
           FROM ${run.table}
          WHERE ${run.map.terapeuta_id}=$1
            AND ${run.map.inicio} >= NOW() - INTERVAL '30 days'
       GROUP BY ${run.map.tipo}
       ORDER BY total DESC`, [terapeutaId]);
            modulos = rows;
        }
        res.json({
...
router.get('/pacientes', requireAuth, async (req, res) => {
    try {
        ...
        const comentariosCountExpr = (cmt.table && cmt.map.sesion_id && ses.table && ses.map.id && ses.map.paciente_id && ses.map.terapeuta_id)
            ? `(
...
router.get('/modulo/:tipo/summary', requireAuth, async (req, res) => {
    try {
        const terapeutaId = req.user?.id;
        if (!terapeutaId)
            return res.status(401).json({ error: 'No autorizado' });
...
router.get('/modulo/:tipo/series', requireAuth, async (req, res) => {
    try {
        const terapeutaId = req.user?.id;
        if (!terapeutaId)
            return res.status(401).json({ error: 'No autorizado' });
...
router.get('/modulo/:tipo/top-pacientes', requireAuth, async (req, res) => {
    try {
        const terapeutaId = req.user?.id;
        if (!terapeutaId)
            return res.status(401).json({ error: 'No autorizado' });
