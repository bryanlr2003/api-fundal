...
router.get('/', async (req, res, next) => {
    try {
        ...
        if (q) {
            ...
            wh.push(`(LOWER(nombre) LIKE $${params.length} OR LOWER(apellido) LIKE $${params.length} OR LOWER(email) LIKE $${params.length})`);
        }
        if (rol) {
            params.push(rol);
...
router.get('/:id', async (req, res, next) => {
    try {
        ...
        const { rows } = await pool.query(`SELECT ${SELECT_COLUMNS} FROM usuarios WHERE id=$1 LIMIT 1`, [id]);
        if (!rows.length)
...
router.post('/', async (req, res, next) => {
    try {
        ...
        // email único
        const dupe = await pool.query('SELECT 1 FROM usuarios WHERE LOWER(email)=LOWER($1) LIMIT 1', [email]);
        if (dupe.rowCount) {
            return res.status(409).json({ error: 'El correo ya está registrado' });
        }
        const hash = await bcrypt.hash(password, 10);
        const { rows } = await pool.query(`INSERT INTO usuarios (rol, nombre, apellido, email, password_hash, activo)
       VALUES ($1,$2,$3,$4,$5,true)
       RETURNING ${SELECT_COLUMNS}`, [rol, nombre, apellido, email, hash]);
...
router.post('/:id/activo', async (req, res, next) => {
    try {
        ...
        const { rows } = await pool.query(`UPDATE usuarios
         SET activo = $1,
             fecha_modifica = NOW()
       WHERE id = $2
       RETURNING ${SELECT_COLUMNS}`, [activo, id]);
        if (!rows.length)
...
router.put('/:id', async (req, res, next) => {
    try {
        ...
        if (email) {
            const dupe = await pool.query('SELECT 1 FROM usuarios WHERE LOWER(email)=LOWER($1) AND id<>$2 LIMIT 1', [email, id]);
            if (dupe.rowCount) {
                return res.status(409).json({ error: 'El correo ya está registrado' });
            }
        }
...
router.delete('/:id', async (req, res, next) => {
    try {
        ...
        const { rows } = await pool.query(`DELETE FROM usuarios
        WHERE id = $1
        RETURNING ${SELECT_COLUMNS}`, [id]);
        if (!rows.length)
            return res.status(404).json({ error: 'Usuario no encontrado' });
